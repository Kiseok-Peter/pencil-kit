...
- **lucide 아이콘** → 플러그인이 unpkg/jsdelivr CDN에서 SVG 받아 벡터화
  - **모든 아이콘을 크기별 컴포넌트**(`DS - Icon Components` 페이지)로 만들고 화면에는 인스턴스만 배치 — 라이브러리에서 재사용·스왑 가능. 인스턴스의 아이콘 교체 오버라이드는 `swapComponent` 로 적용(컴포넌트 연결 유지). 스왑 미지원 환경이면 자동으로 기존 벡터 방식으로 폴백
