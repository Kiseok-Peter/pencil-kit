...
**아이콘 컴포넌트화 + 오버라이드 (인스턴스의 `{icon:"fish"}` 교체)**: Figma 는 인스턴스의 자식 추가/삭제를 금지하므로 벡터를 갈아끼울 수 없다. 플러그인은 디자인에 등장하는 **아이콘마다 단일 마스터**(24px, `DS - Icon Components` 페이지)를 만들고 화면·컴포넌트에는 인스턴스만 리사이즈해 배치한다. 교체 오버라이드는 `swapComponent` 로 적용 — 컴포넌트 연결 유지.
단일 마스터가 가능한 비결: 마스터 생성 시 **`outlineStroke` 로 선을 면으로 굽는다**. 인스턴스 리사이즈는 constraints 스케일이라 live stroke 의 굵기를 스케일하지 않는데(작은 아이콘이 두꺼워짐), 구우면 기하 전체가 비례 스케일되어 기존 rescale 방식과 렌더가 동일하다. `outlineStroke` 미지원 환경은 프로브가 감지해 **크기별 마스터로 자동 폴백**(로그 `아이콘 외곽선화 프로브:`). 스왑 직후 색을 항상 다시 칠한다(오버라이드 fill 또는 슬롯 원래 fill — 스왑의 오버라이드 보존 휴리스틱을 신뢰하지 않음, 색 토큰 바인딩도 함께 복원). **중첩 인스턴스에서 `swapComponent` 가 되는지는 실행 시 프로브가 판정**한다(로그 `아이콘 스왑 프로브:`) — 안 되는 환경이면 마스터를 하나도 만들지 않고 현행(교체 미적용)과 완전히 동일하게 동작한다. 전제: `manifest.json` 에 `documentAccess:"dynamic-page"` 미사용(도입하면 `mainComponent` 동기 getter 가 죽어 이 설계의 동기 빌드 체인이 깨진다).

**안전 설계**: 모든 바인딩·스왑은 리터럴을 정확히 대입한 **뒤에** 시도하고, 직후 다시 읽어 어긋나면 즉시 되돌린다. 최악의 경우에도 결과물이 기능 없는 것과 동일하다 — `node test/verify-bindings.js` 가 이를 기계적으로 증명한다.
