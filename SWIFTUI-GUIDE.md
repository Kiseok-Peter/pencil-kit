...
>
> **비활성 면은 `disabled-bg` 하나입니다** — 버튼·입력(`Text Input - Disabled`·
> `Textarea - Disabled`) 전부. `surface-dark` 는 이제 **Secondary 기본 면 + 중립 회색**만 집니다.
>
> 한동안 두 갈래였습니다(버튼 `disabled-bg` / 입력 `surface-dark`). 다크에서 두 값이 우연히
> 같아(`#2A2A2A`) 갈라진 게 안 보이다가, `disabled-bg` 다크를 `#181818` 로 내리자 **비활성 면이
> 두 회색으로 드러났습니다**(대비 1.000 → 1.237, 라이트 차이 1.054의 2배). 값을 고치면 모양이
> 함께 움직인다는 걸 그때 확인하고 토큰을 합쳤습니다.
>
> → **값이 우연히 같아서 안 보이던 구조 문제는, 값을 고치는 순간 드러납니다.** 색을 옮길 때는
> 그 값을 공유하던 다른 토큰이 무엇이었는지 같이 봐야 합니다.
>
> → 그리고 **참조를 옮기는 변경은 생성물에 안 나타납니다.** 위 통합에서 토큰의 값도 이름도
> 안 바뀌었고 소비 측 생성물(변수 파일·Color Set)은 한 글자도 안 바뀌었는데, 정작 고칠 곳은
> 컴포넌트 코드 한 줄이었습니다. **값이 바뀌면 생성기가 따라오지만 참조가 바뀌면 사람이
> 따라가야 하고, 그 사이에는 아무 게이트도 없습니다.** `diff.py` 가 이걸 따로 찍어줍니다:
>
> ```
> 토큰 참조 이동 4건 — 값이 아니라 어느 토큰을 쓰는가가 바뀌었습니다.
>   · Text Input - Disabled · fill:  $surface-dark → $disabled-bg
> ```
>
