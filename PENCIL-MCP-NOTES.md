...
## 12. MCP 수정은 `.pen` 파일에 자동으로 안 내려간다 — 사람이 저장해야 한다

`execute` 로 고친 내용은 **앱 안에만** 있습니다. 파일은 그대로입니다.

```
초코로드.pen (디스크)   8/3 18:38   ← 이틀치 수정이 안 들어감
Pencil 앱 (메모리)      8/5·8/6 작업 전부 반영됨   ← 추출·검증은 여기서 이뤄진다
```

MCP 도구에 저장이 없고(`get_app_state`/`execute`/`get_screenshot`/`export_nodes`/`export_html`),
`execute` 의 함수 목록에도 없습니다. 자동저장 임시파일도 안 생깁니다(`~/Library` 전수 확인).

**그래서 두 가지가 조용히 어긋납니다.**
- **작업 유실 위험** — 앱이 예기치 않게 닫히면 그 사이 수정이 전부 날아갑니다.
- **저장소가 거짓말을 한다** — 원본은 사흘 전 것인데 산출물은 오늘 것인 상태로 커밋됩니다.
  누가 그 `.pen` 을 열어 재추출하면 산출물과 다른 결과가 나옵니다.

→ **`.pen` 을 고쳤으면 작업 끝에 사용자에게 ⌘S 를 요청하고, 저장을 확인한 뒤 커밋한다.**
확인은 mtime·크기로 합니다(암호화라 내용은 못 봅니다). 저장 뒤 지문 대조를 한 번 더 돌리면
앱 문서와 산출물이 여전히 같은지까지 확정됩니다.

## 13. 방금 만든 노드는 `bounds.y` 가 낡아 있고 스크린샷이 빈 화면으로 나온다
